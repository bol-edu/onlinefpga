...
```
> - `requests`：發送 HTTP 請求至 monitord 服務（租借 / 歸還 FPGA）
> - `pymongo`：連線 MongoDB，存取使用者及設備資料
> - `email-validator`：驗證使用者輸入的電子郵件格式
>
...
> ```
>
> **Ubuntu 24.04：** Python 預設版本為 3.12，預設禁止直接使用 `pip3` 安裝系統層套件，若出現 `externally-managed-environment` 錯誤，請改用：
> ```bash
