...
2. 編輯使用者的 `.bashrc`，加入歡迎畫面及自動啟動 OnlineFPGA 選單：
...
`sync_onlinefpga.sh` 會將 `config.pyc` 及 `onlinefpga.pyc` 複製到 `/home/boleduuser/`，並使用 `chattr +i` 鎖定檔案防止使用者竄改：
